...
public class LegalMoveChecker {
	...
	private static ArrayList<int[][]> moveHistory= new ArrayList<int[][]>();
	private static int libCounter;
		...
		if(coordFound != 0){
			for(int i=1; i<=coordFound; i++){
				...
				for(int o = 0; o< 9; o++){
					for(int q=0; q<9; q++){
						if (libCounter == 0){
							if (board[o][q] == 3) board[o][q] = 0;	
						}
						else {
							if (board[o][q] == 3) board[o][q] = switchPlayer(p);
						}	
					}
				}
				libCounter = 0;
	...
	public static void checkLiberty(int x, int y, int pl, int checked){
		...
		if (board[x][y] == pl) {
			...
			if (x>=1) checkLiberty(x-1, y, pl, checked);
			if (x<=7) checkLiberty(x+1, y, pl, checked);	
			if (y>=1) checkLiberty(x, y-1, pl, checked);
			if (y<=7) checkLiberty(x, y+1, pl, checked);	
		}
	}
