...
%Environment for pseudo-code
%\newenvironment{pseudo_code}{\fontfamily{pseudo_code}\selectfont}{\par}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{Images/S33Diagram.png}
...
The \texttt{LegalMoveChecker} was designed to have a single algorithm that would determine whether a move is legal or not. As our project adheres to the Japanese style Go rules the legality primarily concerns itself with non-suicidal moves (stones cannot be placed only to be captured immediately) and Super Ko (no repetition of previous board states allowed). Accordingly, the following algorithm (and recursive sub-algorithm) was used to return true if the move was legal and false otherwise.

WILL REPLACE THIS WITH LEGALMOVECHECKER PSEUDOCODE.\\
boolean checkMove(colour \texttt{c}, intersection \texttt{i} board and \texttt{b}\\
\begin{algorithm}[H]
\SetAlFnt{\ttdefault}
\While{bananas}{
	\If{apples}{}
